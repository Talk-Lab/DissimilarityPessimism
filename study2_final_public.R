...
p_load(tidyverse, ggplot2, glue, corrr, broom, magrittr, ggExtra, lme4, ggeffects, lmerTest, 
       emmeans, pbkrtest, questionr, knitr, sjPlot, effects, report, brglm, scales, optimx, ggpubr, simr)
...
# exclusions
subset <- subset(d, attention_check == 5)
d <- subset
...
write.csv(d_long, file = glue("{DATA_PATH}/study2_long_aggregate.csv"))
...
#variance calculations for effect size
...
#not collapsed
emm_H2_overall <- emmeans(model, ~ rating_type_c | partner_type_c,
                  at = list(rating_type_c = c(-0.5, +0.5),
                            partner_type_c = c(-0.5, +0.5)))
con_H2_overall <- contrast(emm_H2_overall, list("self - other" = c(-1, 1)), by = c("partner_type_c"))
con_d_H2_overall   <- contrast_to_d(con_H2_overall, sigma_total)
con_H2_overall 
unique(d_long[, c("sim_dissim", "partner_type_c")])
...
#  cell SDs for the full 2x2 (for H2 & H3)
h2_h3_sds <- d_long %>%
  group_by(sim_dissim, self_other, race) %>%
  summarise(M = mean(rating), SD = sd(rating), n_obs = n(), .groups = "drop")
...
### final choice models (see Study 3 for more in depth exploration of choice)

d_long_choice <- d %>%
  pivot_longer(
    cols = c(sim_choice, dissim_choice), 
    names_to = c("partner_sim_dissim_choice"),
    values_to = "choice"
  ) %>%
  mutate(choice = dplyr::recode(choice, "1" = 1, "2" = 0))

subset <- subset(d_long_choice, partner_sim_dissim_choice == "dissim_choice")
d_long_choice <- subset

# Model 1 - other predicting choice
m1_bivariate<- glm(choice ~ dissim_other, data = d_long_choice, family = binomial(link = "logit"))
summary(m1_bivariate )
confint(m1_bivariate)
odds = exp(coef(m1_bivariate))
odds
probability = odds / (1 + odds)
probability
anova(model.dif.1, test="LRT")
sum(table(d_long_choice$choice))
#a one-unit increase in other is associated with a increase in the log odds of saying yes by 0.52 units

# Model 2 - Controlling for self

m2_additive <- glm(choice ~ dissim_self + dissim_other, data = d_long_choice, family = binomial(link = "logit"))
summary(m2_additive)
confint(m2_additive)
odds = exp(coef(m2_additive))
odds
probability = odds / (1 + odds)
probability
anova(m2_additive, test="LRT")
sum(table(d_long_choice$choice))

m3_race_int <- glm(choice ~ dissim_self*black_white + dissim_other*black_white, data = d_long_choice, family = binomial(link = "logit"))
summary(m3_race_int)
report(m3_race_int)
confint(m3_race_int)
odds = exp(coef(m3_race_int))
odds
probability = odds / (1 + odds)
probability
anova(m3_race_int, test="LRT")

simple.slopes <- emtrends(m3_race_int, ~ black_white, var="dissim_other")
test(simple.slopes)
confint(simple.slopes)
#shows that only significant for black
#although note that there wasn't as much variance ot explain as Our paradigm didn't elicit 
#many white people who would not want to interact
