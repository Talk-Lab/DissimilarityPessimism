...
#effects code
#note that this study now has a third factor, so our an overall 2x2 interaction with simple dummy coding, in the simple model output (summary(model)) 
#is not correct because that self_other:sim_dissim interaction is the value at the reference level of general_race
#And H3 is about the interaction overall, not just in one domain.
# -----------------------------------------------
...
# can help reduce over-parameterization and avoids singular fits
fit_rs <- lmer(
  belief ~ rating_type_c * partner_type_c * domain_type_c +
...
h1_sds <- df_subset %>%
  group_by(sim_dissim) %>%
  summarise(M = mean(belief), SD = sd(belief), n_obs = n(), .groups = "drop")
...
#see extra analyses - doesn't really make conceptual sense that less projection for dissimilar can explain this specific gap

#--- Build one row per person × (Similarity × Domain × Conversation) with self/other in columns
df_proj <- df_subset |>
  filter(self_other %in% c("self","other")) |>
  mutate(
    partner_type_c = ifelse(sim_dissim == "sim",  +0.5, -0.5),  # Similarity (±.5)
    domain_type_c  = ifelse(general_race == "race", +0.5, -0.5) # optional if you extend later
  ) |>
  group_by(participant_id, partner_type_c, domain_type_c, conversation) |>
  summarise(
    belief_self  = belief[self_other == "self"],
    belief_other = belief[self_other == "other"],
    .groups = "drop"
  ) |>
  group_by(participant_id) |>
  mutate(                              # person‑mean center -> within‑person projection
    belief_self_c = belief_self - mean(belief_self, na.rm = TRUE)
  ) |>
  ungroup()

#--- Mixed model: projection and its moderation by Similarity
#    Random intercepts + random slope for belief_self_c (uncorrelated)
mod_proj <- lmer(
  belief_other ~ belief_self_c * partner_type_c +
    (1 + belief_self_c || participant_id),
  data    = df_proj,
  REML    = FALSE,
  control = lmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 1e5))
)
...
#--- Simple slopes (projection) by Similarity and their difference
tr_proj <- emtrends(
  mod_proj, ~ partner_type_c, var = "belief_self_c",
  at = list(partner_type_c = c(-0.5, +0.5)),
  pbkrtest.limit = 7856
)
simp_slopes <- summary(tr_proj)        # slope in dissimilar (−.5) and similar (+.5)
diff_slopes <- summary(pairs(tr_proj)) # the moderation test: Similar − Dissim

simp_slopes
diff_slopes

mod_proj_3way <- lmer(
  belief_other ~ belief_self_c * partner_type_c * domain_type_c +
    (1 + belief_self_c || participant_id),
  data    = df_proj,
  REML    = FALSE,
  control = lmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 1e5))
)
summary(mod_proj_3way)

# Simple slopes by Similarity within each Domain:
tr_proj_3 <- emtrends(
  mod_proj_3way, ~ partner_type_c | domain_type_c, var = "belief_self_c",
  at = list(partner_type_c = c(-0.5, +0.5), domain_type_c = c(-0.5, +0.5)),
  pbkrtest.limit = 7856
)
summary(tr_proj_3)
summary(pairs(tr_proj_3), infer = c(TRUE, TRUE))  # slope differences within each domain

rm_dis <- rmcorr(participant_id, belief_self, belief_other,
                 data = dplyr::filter(df_proj, partner_type_c == -0.5))
rm_sim <- rmcorr(participant_id, belief_self, belief_other,
                 data = dplyr::filter(df_proj, partner_type_c == +0.5))
...
run_moderated_parallel_mediation <- function(data_subset, R = 1000, ci_type = "perc") {
  ...
  dat <- data_subset %>%
    mutate(
      rating_type_c  = ifelse(self_other == "self",  +0.5, -0.5),
      partner_type_c = ifelse(sim_dissim == "sim",   +0.5, -0.5),
      M1 = 8 - conf,      # now high = uncertain, matches "Uncertainty" label
      M2 = 8 - valence,   # now high = negative, matches "Negative Valence" label 
      Y  = belief
    )
...
fit_mediation_models <- function(data_subset) {
  dat <- data_subset %>%
    mutate(
      ...
      partner_type_c = ifelse(sim_dissim == "sim",   +0.5, -0.5),
      M1 = 8 - conf,      # now high = uncertain, matches "Uncertainty" label
      M2 = 8 - valence,   # now high = negative, matches "Negative Valence" label  (CONFIRM valence item direction first)
      Y  = belief
    )
...
## 2) Safe wrapper around emtrends() that standardizes column names ----------
